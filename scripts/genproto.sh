...
for dir in ${DIRS}; do
  run pushd "${dir}"
    ...
    run gsed -i.bak -E 's|"etcd/api/|"go.etcd.io/etcd/api/v3/|g' ./**/*.pb.go
    run gsed -i.bak -E 's|"raft/raftpb"|"go.etcd.io/etcd/raft/v3/raftpb"|g' ./**/*.pb.go
    run gsed -i.bak -E 's|"google/protobuf"|"github.com/gogo/protobuf/protoc-gen-gogo/descriptor"|g' ./**/*.pb.go
...
for pb in api/etcdserverpb/rpc server/etcdserver/api/v3lock/v3lockpb/v3lock server/etcdserver/api/v3election/v3electionpb/v3election; do
  ...
  run gsed -i -E "s#package $pkg#package gw#g" "${gwfile}"
  run gsed -i -E "s#import \\(#import \\(\"go.etcd.io/etcd/${pkgpath}\"#g" "${gwfile}"
  run gsed -i -E "s#([ (])([a-zA-Z0-9_]*(Client|Server|Request)([^(]|$))#\\1${pkg}.\\2#g" "${gwfile}"
  run gsed -i -E "s# (New[a-zA-Z0-9_]*Client\\()# ${pkg}.\\1#g" "${gwfile}"
  run gsed -i -E "s|go.etcd.io/etcd|go.etcd.io/etcd/v3|g" "${gwfile}"
  run gsed -i -E "s|go.etcd.io/etcd/v3/api|go.etcd.io/etcd/api/v3|g" "${gwfile}"
  run gsed -i -E "s|go.etcd.io/etcd/v3/server|go.etcd.io/etcd/server/v3|g" "${gwfile}"
...
This API reference is autogenerated from the named \`.proto\` files." || exit 2
  ...
  run gsed -i -e 1,3d ${API_REFERENCE_FILE}
...
This API reference is autogenerated from the named \`.proto\` files." || exit 2
  ...
  # remove the first 3 lines of the doc as an empty --title adds '### ' to the top of the file.
  run gsed -i -e 1,3d ${API_REFERENCE_CONCURRENCY_FILE}
